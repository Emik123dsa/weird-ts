import { Directive, OnInit, Output, OnDestroy, AfterViewInit, TemplateRef, ViewContainerRef, EventEmitter, Optional, Inject, ElementRef } from "@angular/core";
    ...
    setTimeout(() => {
      this.subscribtion = fromEvent<MouseEvent>(this.document, 'click')
        .pipe(filter(event => {
          ...
          return !this.removeDropdownListener(this.element.nativeElement, clickTarget);
        }))
        .subscribe(event => this.outsideClick.emit());
    }, 0);
  }

  protected removeDropdownListener(element: HTMLElement, clickTarget: HTMLElement) {
